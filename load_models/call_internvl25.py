...
def load_internvl25():
    ...
    llm = LLM(model = model_path, tensor_parallel_size=torch.cuda.device_count(), trust_remote_code=True, limit_mm_per_prompt={"image": 10})
...
def call_internvl25(
    ...
    if (label_type == 'single_label'):
        prompt_history[-1]['content'][0]['text'] += output_dict['description']
    else:
        prompt_history[-1]['content'] += output_dict['description']
